...
static void* thread_pool_worker(void* arg) {
    ...
    while (1) {
        ...
        log_message(LOG_INFO, "Thread %ld started task", pthread_self());
        (*(task->function))(task->argument);
        log_message(LOG_INFO, "Thread %ld finished task", pthread_self());
...
int thread_pool_add_task(ThreadPool* pool, void* (*task)(void*), void* arg) {
    ...
    log_message(LOG_INFO, "Task added to pool");
    return 0;
}
